...
static void test_widget_hit_and_context_menu(void) {
	...
	struct tahoe_shell_layout layout;
	tahoe_shell_layout_compute(1920, 1080, false, &config, 2, &layout);
	struct tahoe_rect widget = layout.widgets[0].rect;
	struct tahoe_shell_hit hit = tahoe_shell_hit_test(
		&layout,
		widget.x + widget.width / 2,
		widget.y + widget.height / 2);
	assert(hit.kind == TAHOE_HIT_WIDGET);
	assert(hit.index == 0);
	...
	tahoe_shell_layout_set_context_menu(&layout,
		...
		widget.y + widget.height / 2);
	assert(layout.context_menu_item_count == 5);
	assert(strcmp(tahoe_shell_context_menu_label(
		TAHOE_CONTEXT_MENU_WIDGET, 0), "Edit Widget") == 0);
	assert(strcmp(tahoe_shell_context_menu_label(
		TAHOE_CONTEXT_MENU_WIDGET, 4), "Remove Widget") == 0);
}
